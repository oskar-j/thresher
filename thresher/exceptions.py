...
UNKNOWN_ALGORITHM_NAME = ('Unknown algorithm {name!r}. Available algorithms are: {available}. '
                          'Run get_supported_algorithms() to list them at runtime.')

UNEXPECTED_LABELS = ('Found {unexpected} in "actual_classes", but only -1 and 1 are supported. '
                     'If your data uses different labels, declare them with the "labels" option, '
                     'for example Thresher(labels=(0, 1)).')

SINGLE_CLASS_LABELS = ('"actual_classes" contains only {only}. Both -1 and 1 must be present - '

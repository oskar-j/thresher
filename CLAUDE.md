...
`mkdocs.yml` sets `strict: true`, so a dead cross-reference or a page missing from `nav` fails the build. `docs/changelog.md` is **generated** from `CHANGELOG.md` by the build and by `make docs`; edit the changelog, not that page. Published to GitHub Pages by `.github/workflows/docs.yml` on pushes to `main`, and built (not deployed) on every pull request. `.readthedocs.yaml` exists so Read the Docs can host the same site from the same dependency group if that is ever preferred.

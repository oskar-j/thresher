...
Which link goes where is deliberate: the README points at **GitHub Pages**, which tracks `main`, and the package metadata's `Documentation` URL points at **Read the Docs**, specifically `/en/stable/`, which tracks the newest tag. Someone arriving from a release on PyPI then reads that release's documentation rather than whatever has been merged since. Note the metadata URL is baked in when the distribution is built, so changing it only affects releases made afterwards.
